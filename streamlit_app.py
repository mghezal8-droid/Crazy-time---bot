...
# 🔧 CONFIG INITIALE
# -----------------------------------
st.set_page_config(page_title="🎰 Crazy Time Tracker + EV Live", layout="wide")
...
# -----------------------------------
# 🔎 EV et probabilités pour tous les segments
# -----------------------------------
st.subheader("📊 Probabilités & EV pour tous les segments")
full_history = st.session_state.history + st.session_state.live_history
ev_table = []
for seg in SEGMENTS:
    p = combined_prob(full_history, seg)
    mises = {seg:1.0}  # simuler 1$ sur ce segment
    ev = expected_value_for_strategy(mises, full_history, multiplicateur, st.session_state.bankroll)
    ev_table.append({"Segment": seg, "Probabilité (%)": round(p*100,2), "EV (1$ mise)": round(ev,2)})
st.table(pd.DataFrame(ev_table))

# -----------------------------------
# 📈 Historique Spins + Graphiques
# -----------------------------------
