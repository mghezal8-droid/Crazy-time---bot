    ...
    for r in range(rows):
        ...
        for c in range(cols_per_row):
            if idx >= len(segments):
                break
            seg = segments[idx]
            # clé unique par segment + compteur d'appui pour éviter duplication d'état
...
with act_col2:
    if st.button("🏁 Fin historique", key="btn_fin_hist"):
        ...
        # calcule suggestion initiale et la stocke
        name, mises = None, {}
        if st.session_state.history:
            name, mises = None, {}
            name, mises = None, {}
            # calculer suggestion initiale
            name, mises = None, {}
            name, mises = None, {}
        # on fera la suggestion affichée automatiquement plus bas (fonction choose...)
...
def process_spin_real(spin_result, mises_utilisees, bankroll, mult_real):
    # mult_real can be number or dict; here we expect a uniform top-slot mult value for bonus segments
...
# -------------------------------
# Affichage de la suggestion courante (avant live)
# -------------------------------
st.subheader("📊 Stratégie suggérée (prochaine mise)")
# on calcule suggestion courante basée sur l'historique actuel et on la stocke pour l'utiliser quand on enregistre le spin
if st.session_state.history:
    sugg_name, sugg_mises = choose_strategy_intelligent(st.session_state.history, st.session_state.bankroll)
    # store suggestion to session so that when user hits Enregistrer Spin, we use this mises to compute result
    st.session_state.last_suggestion_name = sugg_name
    st.session_state.last_suggestion_mises = sugg_mises
    st.markdown(f"**Stratégie :** {sugg_name}")
    st.markdown("**Mises proposées :**")
    st.write({k: round(v,2) for k,v in sugg_mises.items()})
else:
    st.write("Entrez l'historique manuel puis appuie sur 'Fin historique' pour que le bot suggère.")

# -------------------------------
# Mode Live (Enregistrer spin) - ordre fixe : on utilise la dernière suggestion stockée pour calculer le résultat,
# puis on ajoute le spin à l'historique et on recalcule la suggestion pour le prochain spin.
# -------------------------------
st.header("Spin Live")
...
with live_col1:
    if st.button("Enregistrer Spin (utilise la dernière suggestion)"):
        # récupération de la dernière suggestion (mises à utiliser pour ce spin)
        mises_for_spin = st.session_state.last_suggestion_mises if st.session_state.last_suggestion_mises else {}
        # si aucune suggestion existante (ex: pas d'historique), on génère une suggestion sur place
        ...
        # calcul du résultat du spin en utilisant les mises_for_spin (qui étaient suggérées AVANT le spin)
        gain_net, mise_total, new_bankroll = process_spin_real(spin_val, mises_for_spin, st.session_state.bankroll, mult_real_for_spin)

        # on enregistre le résultat (on ajoute le spin APRES le calcul)
        ...
        # Après l'ajout du spin, on recalcule et stocke la suggestion POUR LE PROCHAIN SPIN
